...
axios.VERSION = __webpack_require__(6).version;
...
exports.createObject = async (objectType, properties = [], associations = null) => {
...
exports.updateObject = async (objectType, objectId, properties = {}) => {
  const accessToken = process.env.HUBSPOT_API_KEY;
  const endpoint = `https://api.hubapi.com/crm/v3/objects/${objectType}/${objectId}`;
...
exports.deleteObject = async (objectType, objectId) => {
  const accessToken = process.env.HUBSPOT_API_KEY;
  const endpoint = `https://api.hubapi.com/crm/v3/objects/${objectType}/${objectId}`;
...
exports.getProperty = async (objectType, propertyName) => {
  const accessToken = process.env.HUBSPOT_API_KEY;
  const endpoint = `https://api.hubapi.com/crm/v3/properties/${objectType}/${propertyName}`;
...
exports.main = async (context, sendResponse) => {
  try {
    const filterValues = context.body.filterValues;
    const after = context.body.after;
    let limit = context.body.limit;
    const properties = ['firstname', 'lastname', 'hcp_type'];
    const result = await hubspotService.searchObjects('contacts', filterValues, properties, limit, after);
