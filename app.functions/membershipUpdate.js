...
const { constant } = __webpack_require__(96486);
const hubspotService = __webpack_require__(60437);
...
function extractProperties(context) {
  ...
  const properties = fields.reduce((acc, field) => {
    if (context.params[field]?.[0]) {
      acc[field] = context.params[field][0];
    }
    return acc;
  }, {});

  const checkBoxHandling = (fields) => fields.map(field => 
    context.params[field]?.[0] === 'on' && field).filter(Boolean).join(';');
  ...
  properties['membership_status'] = 'Pending';
...
exports.main = async (context, sendResponse) => {
  try {
    ...
    const result = await hubspotService.createObject('contacts', properties);
